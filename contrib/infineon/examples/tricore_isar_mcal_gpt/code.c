...
void GptChannel_0_Notification(void);
void GptChannel_0_Notification(void)
{
  ++gpt_notif_cnt;
}
...
int main(void)
{
  ...
  Gpt_InitCheck(&Gpt_ConfigRoot[0]);
#endif
...
TASK(Task1)
{
  ...
  if ( led_status == STD_LOW ) {
    led_status = STD_HIGH;
    Gpt_StartTimer(GptConf_GptChannel_GptChannelConfiguration_0, 0x1000U);
  }
  else {
    led_status = STD_LOW;
    Gpt_StopTimer(GptConf_GptChannel_GptChannelConfiguration_0);
  }
