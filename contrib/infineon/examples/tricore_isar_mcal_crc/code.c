...
TASK(Task1)
{
  ...
  /* Calling the CRC16 with polynomial = 0x1021 
   * CrcMessage16 is the pointer to the start of the bloc
   * Crc_Length is the size of the block array
   * Crc_StartValue16 is the start value to be used by algorithm
   */
  Crc16Result = Crc_CalculateCRC16 (
    &Crc16Result, 0x0100U, CRC_INITIAL_VALUE16, STD_TRUE
  );
