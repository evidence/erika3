 ...
 * File Name   : r_peid4.c
 * Device(s)   : RH850/E2x
 * Description : project implementation running on PEID4 (core 4), example
 ...
 *****************************************************************************/
#define PRJ_LED_5       g_port_PrjCfg[6].pin
...
/******************************************************************************
 * PE4 state (debug visibility)
 *****************************************************************************/
peid4_state_t g_peid4;
 ...
 *****************************************************************************/
void R_PEID4_HwInit(void)
{
}


/******************************************************************************
 * Function Name : R_PEID4_SwInit
 * Description   : Project initialization.
 * Arguments     : none
 * Return Value  : none
 *****************************************************************************/
void R_PEID4_SwInit(void)
{
...
/******************************************************************************
 * Task Name     : R_PEID4_Task500ms
 * Description   : 500ms tasks
 * Arguments     : none
 * Return Value  : none
 *****************************************************************************/
TASK(R_PEID4_Task500ms)
{
  OsEE_bool   toogle_led;
  StatusType  ev;

  /* task count */
  g_peid4.task500ms_cnt++;

  if (E_OK ==  R_SYNCH_GetPeStatus(OS_CORE_ID_4))
  {
  ...
  R_SYNCH_SetPeStatus(OS_CORE_ID_4, ev);
  R_SYNCH_SetPeValid(OS_CORE_ID_4, OSEE_TRUE);

  toogle_led = R_SYNCH_ManageBarrier();

  if (toogle_led)
  {
    R_PORT_InvertOutLvl(PRJ_LED_5);
  }

  /* Wait for value valid false */
  while (R_SYNCH_GetPeValid(OS_CORE_ID_4));
...
/******************************************************************************
 * Function Name : R_PEID4_RunBckgrndHook
 * Description   : background tasks
 * Arguments     : none
 * Return Value  : none
 * Notes         : background hook shares PE4 load with ERIKA Kernel,
 ...
 *****************************************************************************/
void R_PEID4_RunBckgrndHook(void)
{
  g_peid4.background_cnt++;
}
