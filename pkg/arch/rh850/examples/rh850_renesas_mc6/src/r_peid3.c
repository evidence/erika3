 ...
 * File Name   : r_peid3.c
 * Device(s)   : RH850/E2x
 * Description : project implementation running on PEID3 (core 3), example
 ...
 *****************************************************************************/
#define PRJ_LED_4       g_port_PrjCfg[5].pin
...
/******************************************************************************
 * PE3 state (debug visibility)
 *****************************************************************************/
peid3_state_t g_peid3;
...
/******************************************************************************
 * Task Name     : R_PEID3_Task500ms
 * Description   : 500ms tasks
 * Arguments     : none
 * Return Value  : none
 *****************************************************************************/
TASK(R_PEID3_Task500ms)
{
  OsEE_bool   toogle_led;
  StatusType  ev;

  /* task count */
  g_peid3.task500ms_cnt++;

  if (E_OK == R_SYNCH_GetPeStatus(OS_CORE_ID_3))
  {
  ...
  R_SYNCH_SetPeStatus(OS_CORE_ID_3, ev);
  R_SYNCH_SetPeValid(OS_CORE_ID_3, OSEE_TRUE);

  toogle_led = R_SYNCH_ManageBarrier();

  if (toogle_led)
  {
    R_PORT_InvertOutLvl(PRJ_LED_4);
  }

  /* Wait for value valid false */
  while (R_SYNCH_GetPeValid(OS_CORE_ID_3));
 ...
 *****************************************************************************/
void R_PEID3_RunBckgrndHook(void)
{
  g_peid3.background_cnt++;
}
