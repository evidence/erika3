 ...
 * File Name   : r_peid2.c
 * Device(s)   : RH850/E2x
 * Description : project implementation running on PEID2 (core 2), example
 ...
 *****************************************************************************/
#define PRJ_LED_3       g_port_PrjCfg[4].pin
 ...
 *****************************************************************************/
void R_PEID2_HwInit(void)
{
}


/******************************************************************************
 * Function Name : R_PEID2_SwInit
 * Description   : Project initialization.
 * Arguments     : none
 * Return Value  : none
 *****************************************************************************/
void R_PEID2_SwInit(void)
{
...
/******************************************************************************
 * Task Name     : R_PEID2_Task500ms
 * Description   : 500ms tasks
 * Arguments     : none
 * Return Value  : none
 *****************************************************************************/
TASK(R_PEID2_Task500ms)
{
  OsEE_bool   toogle_led;
  StatusType  ev;

  /* task count */
  g_peid2.task500ms_cnt++;

  if (E_OK == R_SYNCH_GetPeStatus(OS_CORE_ID_2))
  {
  ...
  R_SYNCH_SetPeStatus(OS_CORE_ID_2, ev);
  R_SYNCH_SetPeValid(OS_CORE_ID_2, OSEE_TRUE);

  toogle_led = R_SYNCH_ManageBarrier();

  if (toogle_led)
  {
    R_PORT_InvertOutLvl(PRJ_LED_3);
  }

  /* Wait for value valid false */
  while (R_SYNCH_GetPeValid(OS_CORE_ID_2));
...
/******************************************************************************
 * Function Name : R_PEID2_RunBckgrndHook
 * Description   : background tasks
 * Arguments     : none
 * Return Value  : none
 * Notes         : background hook shares PE2 load with ERIKA Kernel,
 ...
 *****************************************************************************/
void R_PEID2_RunBckgrndHook(void)
{
  g_peid2.background_cnt++;
}
