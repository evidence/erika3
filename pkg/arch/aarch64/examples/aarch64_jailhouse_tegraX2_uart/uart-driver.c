...
#define UART_A_BASE	(uint64_t*)(0x03100000)// /dev/ttyS0
#define UART_B_BASE	(uint64_t*)(0x03110000)// /dev/ttyTHS1
#define UART_C_BASE	(uint64_t*)(0x0c280000)// /dev/ttyTHS2
#define UART_D_BASE	(uint64_t*)(0x03130000)
#define UART_E_BASE	(uint64_t*)(0x03140000)
#define UART_F_BASE	(uint64_t*)(0x03150000)

/* Devices IRQ */
#define UART_A_IRQ	(112)
#define UART_B_IRQ	(113)
#define UART_C_IRQ	(114)
#define UART_D_IRQ	(115)
#define UART_E_IRQ	(116)
#define UART_F_IRQ	(117)
#define IRQ_BASE	(32)
...
static void UART_ISR_UART_E(void);
static void UART_ISR_UART_F(void);
...
/* UART A */
{ .base = UART_A_BASE, .irq = UART_A_IRQ, .isr = UART_ISR_UART_A, .callback =
		NULL},
/* UART B */
{ .base = UART_B_BASE, .irq = UART_B_IRQ, .isr = UART_ISR_UART_B, .callback =
		NULL},
/* UART C*/
{ .base = UART_C_BASE, .irq = UART_C_IRQ, .isr = UART_ISR_UART_C, .callback =
		NULL},
/* UART D*/
{ .base = UART_D_BASE, .irq = UART_D_IRQ, .isr = UART_ISR_UART_D, .callback =
		NULL},
/* UART E*/
{ .base = UART_E_BASE, .irq = UART_E_IRQ, .isr = UART_ISR_UART_E, .callback =
		NULL},
/* UART F*/
{ .base = UART_F_BASE, .irq = UART_F_IRQ, .isr = UART_ISR_UART_F, .callback =
		NULL} };
...
static void uart_read(UART_id_t id) {
	if (id >= 0 && id < MAX_UART_ID) {
		...
		while ((osEE_mmio_read32((uint64_t) device->base + UART_LSR)
				& UART_RX_DATA)) {
			char recv = (char) osEE_mmio_read32(
					(uint64_t) device->base + UART_RX) & 0x000000FF;
...
uint8_t UART_init(UART_id_t id,
	...
	if ((id >= 0) && (id < MAX_UART_ID)) {
		...
		osEE_mmio_write32(base + UART_RX_FIFO_CFG,
		UART_RX_TRIG_VAL | UART_RX_TRIG_ENBL);
		...
		/* Setting the handler */
		CreateTask(&isr_uart_id, OSEE_TASK_TYPE_ISR2, dev->isr, 1U, 1U, 1U,
				OSEE_SYSTEM_STACK);
		/* Tie ISR2 With IRQ */
